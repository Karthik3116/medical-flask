...
# Deep Learning
current_dir = os.getcwd()  # Dynamically get the current working directory
loaded_model = tf.keras.models.load_model(os.path.join(current_dir, "accumodelpp84.h5"))
...
def predict():
    ...
    if file:
        ...
        file.save(temp_path)  # Save the uploaded image file here
...
model = load_model(os.path.join(current_dir, "indianamodel.keras"))


# Load necessary pre-processing objects (wordtoix, ixtoword, max_length)
with open(os.path.join(current_dir,  "ixtoword1.json"), 'r') as f:
    ixtoword = json.load(f)

with open(os.path.join(current_dir,  "wordtoix1.json"), 'r') as f:
    ...
    try:
        ...
        if file:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as temp_file:
                ...
                file.save(temp_path)  # Save the uploaded image to the temp file
